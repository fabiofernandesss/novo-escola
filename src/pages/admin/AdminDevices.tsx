...
const AdminDevices: React.FC = () => {
    ...
    const handleNew = () => {
        ...
        setFormData({ nome: '', tipo: 'Tablet', status: 'Ativo' });
        setIsModalOpen(true);
    };
                ...
                <table className="w-full text-sm">
                    <thead className="bg-gradient-to-r from-gray-50 to-gray-100 border-b">
                        <tr>
                            ...
                            <th className="p-4 font-semibold text-gray-700 text-left">Tipo</th>
                            <th className="p-4 font-semibold text-gray-700 text-left">Modelo</th>
                            <th className="p-4 font-semibold text-gray-700 text-left">Nº Série</th>
                    ...
                    <tbody className="divide-y">
                        {loading ? (
                            <tr><td colSpan={6} className="p-8 text-center"><div className="flex items-center justify-center gap-2"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[hsl(var(--brand-blue))]"></div>Carregando...</div></td></tr>
                        ) : currentItems.length === 0 ? (
                            <tr><td colSpan={6} className="p-8 text-center text-gray-500">Nenhum dispositivo encontrado</td></tr>
                        ) : (
                            currentItems.map((device, i) => (
                                <motion.tr key={device.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-4 text-gray-600">{device.tipo || '-'}</td>
                                    <td className="p-4 text-gray-600">{device.modelo || '-'}</td>
                                    <td className="p-4 text-gray-600">{device.numero_serie || '-'}</td>
                                    <td className="p-4">
                ...
                {totalPages > 1 && (
                    <div className="border-t px-4 py-3 bg-gray-50">
                        <div className="flex items-center justify-between">
                            <p className="text-sm text-gray-600">Mostrando {indexOfFirstItem + 1} a {Math.min(indexOfLastItem, filteredDevices.length)} de {filteredDevices.length}</p>
                            ...
                            <form onSubmit={handleSave} className="p-6 space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select value={formData.tipo || 'Tablet'} onChange={(e) => setFormData({ ...formData, tipo: e.target.value })} className="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-[hsl(var(--brand-blue))] outline-none">
                                ...
                                <div>
                                    ...
                                    <input type="text" value={formData.modelo || ''} onChange={(e) => setFormData({ ...formData, modelo: e.target.value })} className="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-[hsl(var(--brand-blue))] outline-none" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Número de Série</label>
                                    <input type="text" value={formData.numero_serie || ''} onChange={(e) => setFormData({ ...formData, numero_serie: e.target.value })} className="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-[hsl(var(--brand-blue))] outline-none" />
                                </div>
